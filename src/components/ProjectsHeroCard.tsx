...
function ToolCard({ tool }: { tool: typeof TOOLS[0] }) {
  return (
    <div
      ...
      className="group relative h-16 w-32 md:h-20 md:w-36 shrink-0 flex items-center justify-center rounded-2xl bg-surface/80 border border-slate-900/10 shadow-lg hover:border-[#3B82F6]/50 transition-all duration-300 overflow-hidden cursor-pointer backdrop-blur-md"
    >
      <div
      ...
      />
      <span className="absolute bottom-1 text-[9px] font-mono text-slate-900/40 group-hover:text-slate-900 transition-colors">
        {tool.name}
      </span>
    </div>
  )
...
export default function ProjectsHeroCard() {
  ...
  return (
    <div className="px-4 sm:px-6 max-w-[1320px] mx-auto mb-12 relative z-10">
      ...
      <div className="relative w-full rounded-[28px] sm:rounded-[36px] bg-[#0f172a] border border-slate-900/15 shadow-[0_20px_60px_rgba(15,23,42,0.15)] overflow-hidden h-[380px] sm:h-[460px] md:h-[520px] flex flex-col justify-between">
        
        {/* Fond sombre uni */}
        <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
          <div
            className="absolute inset-0"
            style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 20%, rgba(37,99,235,0.05) 0%, transparent 65%)' }}
          />
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/60 border border-[#3B82F6]/40 text-xs font-mono text-clay mb-4 backdrop-blur-md">
          ...
          <p className="text-xs sm:text-sm text-slate-900/80 max-w-lg leading-relaxed mb-6 font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              onClick={playClickSound}
              className="text-xs font-mono text-slate-900/70 hover:text-slate-900 px-4 py-3 rounded-full border border-slate-900/10 hover:border-slate-900/30 backdrop-blur-md transition-colors"
            >
              WhatsApp Direct ↗
            </a>
          </div>
        ...
        <div className="relative z-30 pb-6 px-6 flex justify-center">
          <nav className="flex items-center justify-between sm:justify-start bg-white/80 backdrop-blur-2xl px-2 py-1.5 rounded-full border border-slate-900/15 shadow-2xl max-w-md w-full sm:w-auto gap-2">
            <div className="w-8 h-8 rounded-full bg-white border border-slate-900/20 overflow-hidden shrink-0">
            ...
            <Link
              to="/competences"
              onClick={playClickSound}
              className="hidden sm:block text-xs font-mono font-semibold text-slate-900/70 hover:text-slate-900 px-3 transition-colors"
            >
              Compétences
            </Link>
