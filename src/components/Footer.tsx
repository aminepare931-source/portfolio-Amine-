...
export default function Footer() {
  ...
  const socials = [
    settings.linkedin && { href: settings.linkedin, icon: <Linkedin size={15} />, label: 'LinkedIn' },
    settings.tiktok && { href: settings.tiktok, icon: <TikTokIcon size={14} />, label: 'TikTok' },
    settings.facebook && { href: settings.facebook, icon: <Facebook size={15} />, label: 'Facebook' },
      ...
      <div className="flex items-center gap-3">
        {socials.map((s) => (
          <a
            ...
            title={s.label}
            className="w-8 h-8 rounded-full border border-stroke flex items-center justify-center text-slate-900/60 hover:text-clay hover:border-clay/40 transition-colors"
          >
            {s.icon}
          </a>
        ))}
